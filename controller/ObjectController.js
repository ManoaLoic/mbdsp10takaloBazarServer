...
exports.repostObject = async (req, res) => {
  const { objectId } = req.params;

  try {
    const updatedObject = await ObjectRepository.rePostObject(objectId, req.user.id);
    if (!updatedObject) {
      return res.status(404).json({ error: "Objet non trouvé" });
    }
    if (updatedObject == 1) {
      return res
        ...
        .json({
          error:
            "Vous n'avez pas l'autorisation de republier cet objet, car vous n'en êtes pas le propriétaire.",
        });
    }
    return res
      .status(200)
      .json({
        message: "Statut mis à jour en 'Available'",
        object: updatedObject,
      });
  } catch (error) {
    console.error("Erreur lors de la mise à jour du statut de l'objet:", error);
